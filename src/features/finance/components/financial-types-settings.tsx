...
function TypeForm({ open, kind, item, onClose }: { open: boolean; kind: 'bill' | 'bond'; item: BillTypeDto | BondTypeDto | null; onClose: () => void }) {
  ...
  const cashiers = useResourceListQuery('Cashiers', { Page: 1, PageSize: 100 }, kind === 'bill' && open);
  ...
  const cashierOptions = (cashiers.data?.items ?? []).map((cashier) => ({ label: String(cashier.name ?? cashier.no ?? cashier.id), value: cashier.id }));
